...
export const ConnectionRouteView = ({
  id,
  config,
}: ConnectionRouteViewProps) => {
  ...
  const isNumericType = (type: string): boolean => {
    return config.numericTypes.some((t) => type.toLowerCase().includes(t));
  };

  const isTextType = (type: string): boolean => {
    return config.textTypes.some((t) => type.toLowerCase().includes(t));
  };

  const isDateTimeType = (type: string): boolean => {
    return config.dateTimeTypes.some((t) => type.toLowerCase().includes(t));
  };
  ...
  const getInputTypeForColumn = (
    ...
    if (config.isDateType(column.type)) {
      return "date";
    }
                          ...
                          <div className="m-4 rounded border border-red-200 bg-red-50 p-4 text-red-800 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-200">
                            <div className="flex items-center justify-between">
                              ...
                              <Button
                                ...
                                size="sm"
                                variant={
                                  config.outlineErrorCloseButton
                                    ? "outline"
                                    : undefined
                                }
                              >
