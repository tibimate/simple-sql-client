...
function SQLiteIndexPage() {
  const navigate = useNavigate();
  const [connections, setConnections] = useState<SQLiteConnection[]>([]);
  ...
  useEffect(() => {
    const loadConnections = async () => {
      try {
        ...
        const sqliteConnections = conns.filter(
          (c): c is SQLiteConnection => c.type === "sqlite"
        );
        setConnections(sqliteConnections);
      } catch (error) {
  ...
  const handleCreateConnection = () => {
    navigate({ to: "/sqlite/$id", params: { id: "new" } });
  };

  const handleConnectionClick = (connection: SQLiteConnection) => {
    navigate({ to: `/sqlite/${connection.id}` });
  };
  ...
  return (
    <ScrollArea className="h-screen overflow-y-auto dark:bg-gradient-to-br dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-800">
      {/* Hero Section */}
      <div className="dark:border-zinc-700/50 border-zinc-200 border-b">
          ...
          <div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-center">
            <div className="flex-1">
              ...
              <p className="max-w-2xl text-lg text-zinc-600 dark:text-zinc-400">
                Manage and connect to your SQLite databases. Create new
          ...
          {!isLoading && connections.length > 0 && (
            <div className="grid gap-4">
              {connections.map((connection) => (
                <button
                  className="group rounded-lg border border-zinc-400 bg-zinc-100 dark:border-zinc-700/50 dark:bg-zinc-800/50 p-5 text-left transition-all hover:border-green-500/50 hover:bg-zinc-200 dark:hover:bg-zinc-800 hover:shadow-zinc-300 dark:hover:shadow-green-500/10 hover:shadow-md"
                  key={connection.id}
