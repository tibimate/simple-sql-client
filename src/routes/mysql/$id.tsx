...
function RouteComponent() {
  ...
  const [connection, setConnection] = useState<MySQLConnection | null>(null);
  ...
  const isDateTimeType = (type: string): boolean => {
    const dateTimeTypes = ["timestamp", "date", "time", "datetime", "year"];
  ...
  const handleRenameTable = async (newName: string) => {
    ...
    try {
      ...
      await ipc.client.database.executeQuery({
        connectionId: id,
        query: `RENAME TABLE \`${renamingTable}\` TO \`${newName}\`;`,
      });
